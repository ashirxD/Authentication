...
export default function PatientDashboard() {
  ...
  const fetchAppointments = async (filters = {}) => {
    ...
    try {
      ...
      if (!response.ok) {
        const text = await response.text();
        console.error("[fetchAppointments] Failed:", text);
  ...
  const fetchDoctors = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const text = await response.text();
        console.error("[fetchDoctors] Failed:", text);
  ...
  const fetchMedicalRecords = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const text = await response.text();
        console.error("[fetchMedicalRecords] Failed:", text);
...
const markAllNotificationsAsRead = useCallback(async () => {
  ...
  useEffect(() => {
    ...
    // Catch-all for any unexpected events
  ...
  const renderContent = () => {
    ...
    switch (activeSection) {
      ...
      case "medicalRecords":
        return (
          <ErrorBoundary>
            <MedicalRecordsSection medicalRecords={medicalRecords} />
          </ErrorBoundary>
        );
      case "doctors":
        return (
